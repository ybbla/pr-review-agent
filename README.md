...
面向 Pull Request 的自动代码审查、安全修复与自我进化服务：一条 diff 进来，自动完成风险发现、证据复核、安全修复、反馈学习与版本门禁，并通过 GitHub Webhook 无缝接入仓库工作流。
...
- [API](#api)
- [Skills 的定位与编写方式](#skills-的定位与编写方式)

## 特性

### 审查引擎

- 审查统一 diff，输出结构化问题、修复建议和测试建议（JSON API 与 Markdown 报告）
...
- Webhook HMAC-SHA256 签名校验、delivery 幂等与重放时间窗
- 可选的 PR 评论回写（upsert，不重复创建评论）
...
- SQLite（默认）/ PostgreSQL 持久化任务状态、执行轨迹和最终报告，Redis 生产模式
...
```text
evoagent/
├── api/            # HTTP 服务、路由分发与各域 handler（审查/管理/进化）
├── foundation/     # diff 解析、模型与门禁、LLM 客户端、指标与遥测
├── review/         # Agentic 多 Agent、harness、上下文管理与记忆
├── evolution/      # 提示词进化、Skill 进化与评测 harness
├── repair/         # 安全修复、补丁生成与修复验证
├── infra/          # 鉴权/RBAC、告警、可观测性与灰度发布
├── integration/    # GitHub 客户端与 GitHub App 鉴权
├── storage/        # SQLite/PostgreSQL 存储与 Redis 任务队列
├── experiments/    # 评测实验与进化验证脚本
├── report.py       # Markdown 报告生成
├── service.py      # ReviewService：装配全部子系统的门面
└── config.py       # 环境变量驱动的 Settings
web/                # 管理台前端（登录层 + 任务 Dashboard）
skills/             # 内置 Agent Skill 包（SKILL.md）
tests/              # 单元与集成测试
eval/               # 评测数据集
scripts/            # 评测/实验脚本
Dockerfile、docker-compose.yml    # 容器化与生产依赖编排
```
...
仓库提供 `Dockerfile` 与 `docker-compose.yml`，可一键拉起 EvoAgent 及其生产依赖（PostgreSQL、Redis、Jaeger）：

```powershell
# 先准备好根目录 .env（至少配置 LLM Provider 与 API Key）
docker compose up --build
```

Compose 会以生产模式启动服务：`EVOAGENT_DATABASE_URL` 指向 PostgreSQL、`EVOAGENT_REDIS_URL` 指向 Redis、`EVOAGENT_OTEL_ENDPOINT` 指向 Jaeger，异步 Worker 数为 4，并默认开启登录鉴权。服务端口映射到宿主机 `8080`，Jaeger UI 位于 `http://127.0.0.1:16686`。管理员凭据通过 `EVOAGENT_BOOTSTRAP_ADMIN_USERNAME` / `EVOAGENT_BOOTSTRAP_ADMIN_PASSWORD`（或根目录 `.env`）注入；compose 内置的默认密码与 `EVOAGENT_AUTH_SECRET` 是本地占位值，对外部署前务必替换。

只构建镜像而不启动：`docker build -t evoagent .`。镜像已复制 `evoagent/`、`web/`、`skills/`，`skills` 目录也可在运行时通过 `EVOAGENT_SKILLS_DIR` 重新指定。

## GitHub Webhook

项目使用“GitHub 仓库 Webhook + 公网转发”接收 PR 事件。访问 GitHub 资源（下载 Diff、回写评论、自动修复）默认通过 fine-grained PAT；也可以改用 GitHub App 安装级授权替代 PAT（见下文第 5 节）：
...
### 5. 可选：使用 GitHub App（替代 fine-grained PAT）

除了 fine-grained PAT，也可以用 GitHub App 做安装级授权。Webhook 仍走 `/webhooks/github`，但下载 Diff、回写评论和自动修复改用 App 的安装令牌（installation token），无需为每个仓库单独配置 PAT。先在 GitHub 创建 App，再配置：

```powershell
$env:EVOAGENT_GITHUB_APP_ID = '<App ID>'
$env:EVOAGENT_GITHUB_APP_SLUG = '<App slug>'
$env:EVOAGENT_GITHUB_PRIVATE_KEY_PATH = '<GitHub App 私钥 .pem 文件路径>'
$env:EVOAGENT_PUBLIC_BASE_URL = 'https://<公网域名>'
python -m evoagent
```

- `EVOAGENT_GITHUB_PRIVATE_KEY_PATH` 指向 GitHub App 设置页生成的 RSA 私钥文件；该模式依赖 `PyJWT[crypto]`（已在 `requirements.txt` 中）；
- 在 GitHub App 设置页把 **Callback URL / Setup URL** 指向 `https://<公网域名>/github/setup`；
- 打开 `https://<公网域名>/github/install`（或直接访问 App 的安装页）把 App 安装到目标仓库，GitHub 会回调 `/github/setup`，服务记录 installation→租户 的映射后跳转回管理台；
- Webhook 事件负载带有 `installation.id` 时，服务自动用对应安装令牌访问仓库。若要回写评论或自动修复，App 需具备 `Pull requests: Read and write`、`Contents: Read and write` 权限，并订阅 `Pull request` 事件。

GitHub App 与 PAT 可以同时配置；带 `installation.id` 的任务优先使用 App 安装令牌。自动修复只覆盖可确定安全的规则（例如调试输出、`shell=True` 和硬编码 Python 凭据），结果始终提交到新的 `evoagent/fix-pr-*` 分支，不直接修改源分支。
...
`POST /v1/reviews` 的 `diff` 最大默认 1 MiB；单任务默认最多 8 步、120 秒，可通过环境变量调整（详见 `.env.example`）。请求体除 `repository`、`pull_request`、`mode`、`diff` 外，还支持 `repository_root`（本地仓库绝对路径，用于仓库级取证）、`enabled_agents`（覆盖默认启用的 Agent 角色集合）与 `enabled_skills`（限定本次使用的 Agent Skill）。`mode` 目前只接受 `agentic`。
...
| `GET` | `/github/install` | 跳转到 GitHub App 安装页（需配置 App slug） |
| `GET` | `/github/setup` | GitHub App 安装回调，记录 installation→租户 映射 |
...
| `GET` | `/api/dashboard` | 管理台概览：统计、最近任务、队列与编排器状态 |
| `GET` | `/api/tasks` | 分页列出本租户任务 |
| `GET` | `/api/skills` | 列出内置扫描器与已激活的 Agent Skill |
| `GET` | `/api/failures` | 列出失败案例（需 audit 权限） |
